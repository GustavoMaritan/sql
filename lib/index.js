...
class Sql {
	constructor(config) {
		...
		for (let type in sql.TYPES) {
			...
			this.types[typeName] = setTypes(type);
		}
	}
	...
	async mock(procedure, parameters) {
		...
		if (!exists)
			throw {
				message: {
					code: 1,
					messages: [`Arquivo mocha.json não encontrado para ${filename}`]
				}
			};
		...
		if (erros.length)
			throw { message: { code: 2, messages: erros } };
...
function setTypes(type) {
	switch (type) {
		case 'Decimal':
		case 'Numeric':
			return (precision, scale, value = null) => {
				return {
					type: sql[type](precision, scale),
					value: value
				};
			};
		case 'Char':
