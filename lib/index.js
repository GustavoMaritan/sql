...
class Sql {
	...
	async mock(procedure) {
		...
		if (!exists)
			throw { message: `Arquivo mocha.json não encontrado para ${filename}\n { "procedure": { "content":null, "outputs":null, "returnValue":null }` };
		...
		for (const key in json.parameters) {
			if (json.parameters[key] != json[key])
				erros.push(`${key}`);
		}

		if (erros.length)
			throw { message: `Parâmetros inválidos - ${erros.join(',')}` };
